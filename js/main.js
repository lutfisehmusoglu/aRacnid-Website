...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const translations = {
    en: {
      ...
      'download.subtitle': 'Microsoft Store is recommended for most users. All packages are built for Windows 10 / 11 64-bit.',
      'download.recommended': 'Recommended',
      'download.githubTitle': 'GitHub',
      'download.githubDesc': 'Get the latest release setup file straight from the source repository.',
    ...
    tr: {
      ...
      'download.subtitle': 'Çoğu kullanıcı için Microsoft Store önerilir. Tüm paketler Windows 10 / 11 64-bit içindir.',
      'download.recommended': 'Önerilen',
      'download.githubTitle': 'GitHub',
      'download.githubDesc': 'En son sürümün setup dosyasını doğrudan kaynak deposundan al.',
  ...
  // Direct URL generated by https://apps.microsoft.com/badge for this product.
  // Microsoft resolves the current Web Installer; never pin the resulting EXE URL.
  // Keep this separate from the existing Portable download handler.
  const platform = navigator.userAgentData?.platform || navigator.platform || '';
  if (!/win/i.test(platform)) {
    document.querySelectorAll('.store-download').forEach(link => {
      link.href = 'https://apps.microsoft.com/detail/9P63JVQ9ZTX2';
    });
  }
